...
- **Standing master-to-VPS instruction (2026-09-15):** when the user authorizes a push to `master` for this repository, finish the workflow by deploying the reviewed changes to the corresponding SocialPro VPS services and verifying the affected public journey. Commit/push alone is not completion. Preserve runtime configuration and data, deploy only affected services, retain rollback, and verify checks for the exact release. A later explicit “do not deploy” overrides this default. If a gate fails, fix it or report the concrete blocker; never claim Git and production are aligned without checking the active revision.

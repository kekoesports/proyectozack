...
describe('twitch service', () => {
  ...
  describe('fetchTwitchFollowerCounts', () => {
    ...
    it('omits an unavailable broadcaster from the legacy follower-count map', async () => {
      const mock = global.fetch as jest.Mock;
      // Token
      mock.mockResolvedValueOnce(STALE_TOKEN_RESP);
    ...
    it('omits the unknown count when the follower API rejects a request', async () => {
      const mock = global.fetch as jest.Mock;
      // Token
      mock.mockResolvedValueOnce(STALE_TOKEN_RESP);
  ...
  describe('getCS2LiveStreams', () => {
    ...
    it('returns [] when API returns empty data array', async () => {
      mockFetch(makeResponse({ data: [], pagination: {} }));
      const result = await getCS2LiveStreams();
      expect(result).toEqual([]);
    });
    it('throws when streams API returns non-OK', async () => {
      ...
      await expect(getCS2LiveStreams()).rejects.toThrow(
        'Twitch streams API coverage unavailable',
      );
    });
  });
  // ── getTwitchChannelInfo ───────────────────────────────────────────────────
  describe('getTwitchChannelInfo', () => {
    ...
    it('retains null followers for a channel whose follower fetch fails', async () => {
      const mock = global.fetch as jest.Mock;
      // Token
      mock.mockResolvedValueOnce(STALE_TOKEN_RESP);
      // Channels endpoint
      ...
      expect(ch1?.followerCount).toBeNull();
      // b-2 still populated
      expect(ch2?.followerCount).toBe(500);
    });
    it('retains null followers for a channel whose follower API rejects a request', async () => {
      const mock = global.fetch as jest.Mock;
      // Token
      mock.mockResolvedValueOnce(STALE_TOKEN_RESP);
      // Channels endpoint

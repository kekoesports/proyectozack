...
type Props = {
  ...
  readonly summary: CampaignBrandSummary;
};
...
/**
 * Tab dentro del CRM de marcas que lista las campañas asociadas a una marca con su estado y resumen agregado.
 *
 * @kind client
 * @feature admin/brands
 * @route /admin/brands
 */
export function BrandCampaignsTab({ brandId, campaigns, summary }: Props): React.ReactElement {
  if (campaigns.length === 0) {
    return (
      <div className="rounded-xl bg-sp-admin-card border border-sp-admin-border">
        <EmptyState
          title="Sin campañas"
          description="Esta marca no tiene campañas registradas todavía."
          action={
            <Link
              href={`/admin/campanas?brandId=${brandId}`}
    ...
    <div className="rounded-xl bg-sp-admin-card border border-sp-admin-border overflow-hidden">
      <div className="overflow-x-auto">
        <table className="w-full text-xs">
          ...
          <tbody>
            {campaigns.map((c) => {
              const amountBrand = Number(c.amountBrand);
              const amountTalent = Number(c.amountTalent);
              const commission = amountBrand - amountTalent;
              ...
              return (
                ...
                >
                  ...
                  <td className="px-4 py-3 text-right font-mono text-sp-admin-muted">
                    {fmt(commission)}
                  </td>
      ...
      <div className="flex items-center justify-between px-4 py-3 border-t border-sp-admin-border bg-sp-admin-bg/30">
        <p className="text-xs text-sp-admin-muted">
          <span className="font-semibold text-sp-admin-text">Total facturado: {fmt(summary.totalAmountBrand)}</span>
          {' · '}
          <span>Comisión total: {fmt(summary.totalCommission)}</span>
          {' · '}

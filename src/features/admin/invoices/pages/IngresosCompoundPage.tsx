...
/**
 * Página compuesta de "Ingresos" — movimientos, facturas emitidas,
 * clientes de facturación y empresas emisoras.
 *
 * Se comparte entre `/admin/finanzas/ingresos` (ruta canónica del
 * rediseño) y — vía redirect — el legacy `/admin/facturacion`. Ver
 * `docs/finanzas-audit.md` §14.
 *
 * Extraído de la antigua `src/app/admin/(dashboard)/facturacion/page.tsx`
 * para poder migrar la ruta sin duplicar código ni romper bookmarks.
 */
...
export async function IngresosCompoundPage({ headerTitle = 'Ingresos', headerSubtitle = 'Movimientos financieros y facturas emitidas' }: { headerTitle?: string; headerSubtitle?: string } = {}): Promise<React.ReactElement> {
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-start justify-between gap-4 flex-wrap">
        <div>
          <h1 className="text-xl font-bold text-sp-admin-text leading-none">{headerTitle}</h1>
          <p className="text-[11px] text-sp-admin-muted mt-1">{headerSubtitle}</p>
        </div>

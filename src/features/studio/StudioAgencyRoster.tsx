...
export function StudioAgencyRoster({ roster }: { roster: { id: number; name: string; photoUrl: string | null; game: string; status: string }[] }) {
  ...
  return <section className="studio-agency-roster"><div className="studio-section-title"><div><p className="studio-eyebrow">MISMA BASE / SIN DUPLICAR FICHAS</p><h2>Los talentos de SocialPro.</h2></div><span className="studio-badge">{roster.length} fichas CRM</span></div>
    ...
    <div className="studio-roster-grid">{visible.slice(0, limit).map((talent) => <article key={talent.id} className="studio-roster-entry"><Link href={`/admin/talents/${talent.id}`} className="studio-roster-card">{talent.photoUrl ? <Image src={talent.photoUrl} alt="" width={64} height={64} unoptimized /> : <span className="studio-roster-initial">{talent.name.slice(0, 1)}</span>}<div><strong>{talent.name}</strong><small>{talent.game}</small></div><ArrowUpRight size={16} /></Link><StudioWorkspaceButton talentId={talent.id} name={talent.name} /></article>)}</div>
    ...
    <p className="studio-muted">La agencia puede preparar contenido con su propia cuenta. Para que el creador entre, vincula su correo verificado mediante invitación; abrir Studio no modifica sus accesos.</p>
  </section>;
}

...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  return {
    ...
    alternates: { canonical: `/c/${creatorSlug}` },
    openGraph: {
      title,
      description,
...
export default async function RootCreatorPage({ params }: PageProps): Promise<React.JSX.Element> {
  ...
  const totalFollowersLine = talent.socials.map((s) => `${s.followersDisplay} ${s.platform}`).join(' · ');
    ...
    <>
      ...
      <header className="sticky top-0 z-50 bg-sp-black/90 backdrop-blur-xl border-b border-white/[0.04]">
        <div className="max-w-7xl mx-auto px-6 h-14 flex items-center justify-between">
          ...
          <span className="text-[10px] font-bold uppercase tracking-[0.2em] text-white/30">Perfil de creador</span>
        </div>
      </header>
      ...
      <section className="relative overflow-hidden border-b border-white/[0.04]">
        <div
          ...
          style={{ background: `radial-gradient(60% 50% at 50% 0%, ${talent.gradientC1}33 0%, transparent 70%)` }}
          aria-hidden
        />
            ...
            <div className="mt-5 flex flex-col sm:flex-row items-center sm:items-start gap-3">
              ...
              {codesWithTalent.length > 0 && (
                <p className="text-[11px] font-bold uppercase tracking-wider text-white/40">
                  {codesWithTalent.length} {codesWithTalent.length === 1 ? 'código activo' : 'códigos activos'} · Apoya a {talent.name} usando sus códigos
                </p>
              )}
            </div>
          </div>

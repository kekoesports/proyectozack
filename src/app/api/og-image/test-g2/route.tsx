...
// Identical to test-combo but with giveaway-like SQL via db.$client
export async function GET(req: Request) {
  try {
    ...
    return new ImageResponse(
      <div style={{ width: '100%', height: '100%', background: '#050507', display: 'flex', alignItems: 'center', justifyContent: 'center', flexDirection: 'column', gap: '16px' }}>
        ...
        <div style={{ fontSize: 14, color: c2 }}>socialpro.es</div>
      </div>,
      { width: 1200, height: 630 },
    );
  } catch (err) {
    const msg = err instanceof Error ? err.message : String(err);
    return new Response(`TEST_G2_ERROR: ${msg}`, { status: 500, headers: { 'content-type': 'text/plain' } });
  }
}

...
// test-g4 JSX but force-consuming the stream to catch Satori errors inside try/catch
  ...
  try {
    ...
    if (!isNaN(id)) {
      const rows = await db.$client`
        SELECT g.title, g.brand_name, t.name AS talent_name, t.gradient_c1, t.gradient_c2
        FROM giveaways g INNER JOIN talents t ON t.id = g.talent_id WHERE g.id = ${id} LIMIT 1
      `;
      const g = rows[0];
      if (g) {
        ...
        <div style={{ width: '100%', height: '100%', background: '#050507', display: 'flex', flexDirection: 'column', fontFamily: 'sans-serif', position: 'relative' }}>
          ...
          <div style={{ flex: 1, display: 'flex', alignItems: 'center', gap: 56, padding: '56px 72px 40px 80px' }}>
            <div style={{ width: 220, height: 220, borderRadius: 20, flexShrink: 0, background: `linear-gradient(135deg,${c1},${c2})`, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
              <span style={{ fontSize: 72, fontWeight: 900, color: 'white' }}>{brandName.slice(0, 2).toUpperCase()}</span>
            </div>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 14, flex: 1, minWidth: 0 }}>
              <div style={{ fontSize: 13, color: c1, letterSpacing: 5, textTransform: 'uppercase', fontWeight: 700 }}>SocialPro · Sorteo</div>
              <div style={{ fontSize: titleSize, fontWeight: 900, color: '#ffffff', lineHeight: 1.05, letterSpacing: -1, textTransform: 'uppercase', overflow: 'hidden' }}>{title}</div>
          ...
          <div style={{ padding: '0 72px 24px 80px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <div style={{ fontSize: 12, color: 'rgba(255,255,255,0.18)', letterSpacing: 2 }}>socialpro.es/giveaways</div>
    ...
    // Force stream consumption — catches Satori render errors inside the try/catch
    ...
    return new Response(`TEST_G5_ERROR: ${msg}`, { status: 500, headers: { 'content-type': 'text/plain' } });
  }
}

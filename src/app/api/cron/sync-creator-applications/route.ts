...
import { markCreatorOutreachNoResponse, reconcileCreatorOutreachDeliveryEvents } from '@/lib/queries/creatorOutreach';
...
export async function GET(request: NextRequest): Promise<NextResponse> {
  ...
  try {
    const deliveryReconciled = await reconcileCreatorOutreachDeliveryEvents();
    ...
    console.log('[sync-creator-applications] done', { ...result, deliveryReconciled, noResponse, outreach });
    return NextResponse.json({ ok: true, ...result, deliveryReconciled, noResponse, outreach });
  } catch (error) {

...
export async function upsertTargetsFromCSV(
  ...
  const values = rows.map((r) => {
    ...
    return {
      ...
      lastVideoAt: r.last_video_at ?? null,
      recentVideoCount: r.recent_video_count ?? null,
      minRecentVideoViews: r.min_recent_video_views ?? null,
      avgRecentVideoViews: r.avg_recent_video_views ?? null,
      recentVideosWindowDays: r.recent_videos_window_days ?? null,
      qualificationUpdatedAt: r.recent_video_count === undefined ? null : new Date(),
      complianceActivity: r.compliance_activity ?? null,
      complianceStatus: r.compliance_status ?? null,
      complianceSourceUrl: r.compliance_source_url ?? null,
      complianceCheckedAt: r.compliance_checked_at ?? null,
      contactEmail: r.contact_email ?? null,
      contactUrl: r.contact_url ?? null,
      qualificationStatus: r.qualification_status ?? 'review',
      fitScore: r.fit_score ?? 0,
      fitReasons: r.fit_reasons ?? [],
      sourceQuery: r.source_query ?? null,
  ...
  const result = await db
    ...
    .onConflictDoUpdate({
      ...
      set: {
        ...
        complianceActivity: sql`coalesce(excluded.compliance_activity, ${targets.complianceActivity})`,
        complianceStatus: sql`coalesce(excluded.compliance_status, ${targets.complianceStatus})`,
        complianceSourceUrl: sql`coalesce(excluded.compliance_source_url, ${targets.complianceSourceUrl})`,
        complianceCheckedAt: sql`coalesce(excluded.compliance_checked_at, ${targets.complianceCheckedAt})`,
...
export async function createTarget(data: CreateTargetInput): Promise<Target> {
  const [row] = await db
    ...
    .values({
      ...
      lastVideoAt: data.lastVideoAt ?? null,
      recentVideoCount: data.recentVideoCount ?? null,
      minRecentVideoViews: data.minRecentVideoViews ?? null,
      avgRecentVideoViews: data.avgRecentVideoViews ?? null,
      recentVideosWindowDays: data.recentVideosWindowDays ?? null,
      qualificationUpdatedAt: data.qualificationUpdatedAt ?? null,
      complianceActivity: data.complianceActivity ?? null,
      complianceStatus: data.complianceStatus ?? null,
      complianceSourceUrl: data.complianceSourceUrl ?? null,
      complianceCheckedAt: data.complianceCheckedAt ?? null,
      contactEmail: data.contactEmail ?? null,
      contactUrl: data.contactUrl ?? null,
      qualificationStatus: data.qualificationStatus ?? 'review',
      fitScore: data.fitScore ?? 0,
      fitReasons: data.fitReasons ?? [],
      sourceQuery: data.sourceQuery ?? null,
      lastActivityAt: data.lastActivityAt ?? null,
      lastDiscoveredAt: data.lastDiscoveredAt ?? new Date(),
...
export async function bulkUpsertTargets(
  ...
  const values = rows.map((r) => ({
    ...
    defaultLanguage: r.defaultLanguage ?? null,
    lastVideoAt: r.lastVideoAt ?? null,
    recentVideoCount: r.recentVideoCount ?? null,
    minRecentVideoViews: r.minRecentVideoViews ?? null,
    avgRecentVideoViews: r.avgRecentVideoViews ?? null,
    recentVideosWindowDays: r.recentVideosWindowDays ?? null,
    qualificationUpdatedAt: r.qualificationUpdatedAt ?? null,
    complianceActivity: r.complianceActivity ?? null,
    complianceStatus: r.complianceStatus ?? null,
    complianceSourceUrl: r.complianceSourceUrl ?? null,
    complianceCheckedAt: r.complianceCheckedAt ?? null,
    contactEmail: r.contactEmail ?? null,
    contactUrl: r.contactUrl ?? null,
    qualificationStatus: r.qualificationStatus ?? 'review',
    fitScore: r.fitScore ?? 0,
    fitReasons: r.fitReasons ?? [],
    sourceQuery: r.sourceQuery ?? null,
    lastActivityAt: r.lastActivityAt ?? null,
    lastDiscoveredAt: r.lastDiscoveredAt ?? new Date(),
